...
Also, you may use our repo2docker offered by mybinder.org, which encapsulates the algorithm and all required data in one virtual machine instance. All Jupyter Notebook examples found in this repository will be also available to you in action to practice in this respo2docer. Thanks mybinder.org, you made it possible!
...
Simply install DenMune clustering algorithm using pip command from the official Python repository
  ...
  From the Jupyter Notebook cell run the command
...
*<u>Please note that the first denmune (the package) is in small letters, while the other one(the class itself) has D and M in capital case</u>.*

## Read data

There are four possible cases of data:

  - only training data without labels
...
This is an intuitive dataset that has no groundtruth provided
...
- train_data:
  ...
  - default: None. It should be provided by the user, otherwise, an error will arise.
...
- rgn_tsn:
  ...
  - when set to True: It will regenerate the reduced 2-D version of the N-D dataset each time the algorithm runs.
  - when set to False: It will generate the reduced 2-D version of the N-D dataset the first time only, then will reuse the saved existing file
  - default: True
  
- file_2d: name (include location) of file used to save/load the reduced 2-d version

  - if empty: the algorithm will create a temporary file named '_temp_2d'
  - default: None
...
The algorithm provides an exploratory tool called an analyzer, once called it will provide you with an in-depth analysis of how your clustering results perform.
...
DenMune detects noise and outliers automatically, no need for any further work from your side.

  - It plots pre-identified noise in black
  - It plots post-identified noise in light grey

You can set the show_noise parameter to False. 
...
The following chart shows the evolution of pre and post-identified noise in correspondence to the increase of the number of knn. Also, the detected number of clusters is analyzed in the same chart concerning both types of identified noise.
...
## The Stability
The algorithm is only a single parameter, and even more, it is not sensitive to changes in that parameter, k. You may guess that from the following chart yourself. This is of great benefit to you as a data exploration analyst. You can simply explore the dataset using an arbitrary k. Being Non-sensitive to changes in k, make it robust and stable.
...
One of the top-performing features of this algorithm is enabling you to watch how your clusters propagate to construct the final output clusters. Just use the parameter 'prop_step' as in the following example:
...
# create a list of different snapshots of the propagation
...
This notebook allows you to interact with the algorithm in many aspects:

- you can choose which dataset to cluster (among 4 chameleon datasets)
- you can decide which number of k-nearest neighbors to use
...
- If you are a kaggler like me, then Kaggle, the best workspace where data scientists meet, should fit you to test the algorithm with great experience.
...
abstract = {Many clustering algorithms fail when clusters are of arbitrary shapes, of varying densities, or the data classes are unbalanced and close to each other, even in two dimensions. A novel clustering algorithm, “DenMune” is presented to meet this challenge. It is based on identifying dense regions using mutual nearest neighborhoods of size K, where K is the only parameter required from the user, besides obeying the mutual nearest neighbor consistency principle. The algorithm is stable for a wide range of values of K. Moreover, it can automatically detect and remove noise from the clustering process as well as detect the target clusters. It produces robust results on various low and high-dimensional datasets relative to several known state-of-the-art clustering algorithms.}
}
```
...
```
Abbas, M. A., El-Zoghabi, A., & Shoukry, A. (2023). PyMune: A Python package for complex cluster detection. Software Impacts, 17, 100564. https://doi.org/10.1016/j.simpa.2023.100564
```
